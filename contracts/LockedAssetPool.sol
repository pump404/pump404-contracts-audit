...
contract LockedAssetPool is ILockedAssetPool, Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    modifier onlyLaunchHub() {
        require(msg.sender == launchHubAddress, "LockedAssetPool: caller is not the launch contract");
        _;
    }
    ...
    function setLaunchHubAddress(address launchAddress_) external onlyOwner {
        require(launchAddress_ != address(0), "LockedAssetPool: launch address is the zero address");
        launchHubAddress = launchAddress_;
    }
    ...
    function approveAsset(address asset_) external override onlyLaunchHub {
        IERC20 asset = IERC20(asset_);

        asset.approve(launchHubAddress, type(uint256).max);
    }
    ...
    */
    function sendToUniswapV3(address tokenAddress_, uint160 sqrtPriceX96_, int24 tickLower, int24 tickUpper) external onlyLaunchHub returns(uint256 tokenId, uint128 liquidity, address v3_pool, uint256 amount0, uint256 amount1, uint256 ignition_fee) {
        require(address(nonfungiblePositionManager) != address(0), "Launch: nonfungible position manager is the zero address");

        ILaunchHub launch_hub = ILaunchHub(launchHubAddress);

        IAssetPool asset_pool = IAssetPool(tradingHub.tokenInAssetPool(tokenAddress_));
        require(address(asset_pool) != address(0), "LockedAssetPool: token is not launched");
        ...
        uint256 send_to_uniswap = asset_pool.MAX_RESERVE_BALANCE() - launch_hub.ignitionFee();

        IERC20 token = IERC20(tokenAddress_);
        IWETH9 weth = IWETH9(nonfungiblePositionManager.WETH9());

        uint256 token_balance = token.balanceOf(address(this));
        require(token_balance > 0, "LockedAssetPool: insufficient token balance");

        uint24 swap_fee = 10000;
        ...
        v3_pool = nonfungiblePositionManager.createAndInitializePoolIfNecessary(token0, token1, swap_fee, sqrtPriceX96_);
        ...
        (bool success, ) = launch_hub.treasuryAddress().call{value: ignition_fee}("");
        require(success, "LockedAssetPool: failed to transfer remaining balance");
    }

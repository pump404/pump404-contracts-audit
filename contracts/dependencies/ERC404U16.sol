...
abstract contract ERC404U16 is IERC404 {
  ...
  uint256 public constant ID_ENCODING_PREFIX = 0; //1 << 255;

  uint256 public constant MAX_TOTAL_SUPPLY = 1_000_000_000 * 10 ** 18;

  uint16 private _erc721TotalSupply;

  constructor(string memory name_, string memory symbol_, uint8 decimals_, uint256 ratio_) {
    name = name_;
    symbol = symbol_;
  ...
  function _isValidTokenId(uint256 id_) internal view returns (bool) {
    return id_ > 0 && id_ <= _erc721TotalSupply;
...
//    }
    ...
    if (totalSupply + value_ > MAX_TOTAL_SUPPLY) {
      revert('MintLimitReached()');
    }

    _erc721TotalSupply += uint16(value_ / units);
